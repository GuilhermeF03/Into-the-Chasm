...
@export var range_min_var: StringName = &"range_min"

## Maximum distance to the desired position.
@export var range_max_var: StringName = &"range_max"
...
func _tick(_delta: float) -> Status:
	...
	var range_min = blackboard.get_var(range_min_var)
	var range_max = blackboard.get_var(range_max_var)

	while not is_good_position:
		# Randomize until we find a good position (good position == not outside the arena).
		var angle: float = randf() * TAU
		var rand_distance: float = randf_range(
			range_min, 
			range_max
		)
