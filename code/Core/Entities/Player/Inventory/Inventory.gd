...
@onready var pages: MarginContainer = $"Background Panel/Outer Margin/UI/Content"
#endregion
...
func open():
	...
	var page_name: String = get_page(current_page)
...
func leaf(next_page : int):
	...
	var page_name: String = get_page(next_page)
	for page: HBoxContainer in pages.get_children():
		page.visible = (page.name == page_name)
